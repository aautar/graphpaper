...
var computeConnectorPath=function computeConnectorPath(a,b,c){var d=a.anchor_start_centroid.split(" "),e=a.anchor_end_centroid.split(" "),f=new Point(parseFloat(d[0]),parseFloat(d[1])),g=new Point(parseFloat(e[0]),parseFloat(e[1])),h=b.findDistanceToPointClosestTo(f),i=b.findPointsCloseTo(f,h).findPointClosestTo(g),j=b.findPointsCloseTo(g,h).findPointClosestTo(f),k=c.computeRoute(i,j),l=k.toArray(),m=[f].concat(_toConsumableArray(l),[g]);return{svgPath:SvgPathBuilder.pointsToPath(m),pointsInPath:m}}; var convertArrayBufferToFloat64Array=function convertArrayBufferToFloat64Array(a){return new Float64Array(a)};onmessage=function onmessage(a){var b={overallTime:null},c=new Date,d=a.data.gridSize,e=a.data.connectorDescriptors,f=new PointSet(convertArrayBufferToFloat64Array(a.data.routingPoints)),g=new LineSet(convertArrayBufferToFloat64Array(a.data.boundaryLines)),h=new PointSet(convertArrayBufferToFloat64Array(a.data.routingPointsAroundAnchor)),i=new PointVisibilityMap(f,g);e.forEach(function(a){var b=computeConnectorPath(a,h,i),c=new PointSet(b.pointsInPath);a.svgPath=b.svgPath, a.pointsInPath=c.toFloat64Array().buffer;}), b.overallTime=new Date-c, b.numRoutingPoints=f.count(), b.numBoundaryLines=g.count(), postMessage({connectorDescriptors:e,metrics:b});};
