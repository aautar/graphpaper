...
var pointToSvgLineTo=function pointToSvgLineTo(a){return"L"+a.getX()+" "+a.getY()}; var computeConnectorSvg=function computeConnectorSvg(a,b,c){var d=a.anchor_start_centroid.split(" "),e=a.anchor_end_centroid.split(" "),f=new Point(d[0],d[1]),g=new Point(e[0],e[1]),h=b.findDistanceToPointClosestTo(f),i=b.findPointsCloseTo(f,h).findPointClosestTo(g),j=b.findPointsCloseTo(g,h).findPointClosestTo(f),k=c.computeRoute(i,j),l=k.toArray(),m=[];l.forEach(function(a){m.push(pointToSvgLineTo(a));}), m.push(pointToSvgLineTo(g));var n=f.getX()+" "+f.getY(),o="M"+n+m.join(" ");return o};onmessage=function onmessage(a){var b={overallTime:null},c=new Date,d=a.data.gridSize,e=a.data.connectorDescriptors,f=a.data.routingPoints,g=new Float64Array(f),h=new PointSet(g),i=a.data.boundaryLines,j=new Float64Array(i),k=new LineSet(j),l=a.data.routingPointsAroundAnchor,m=new Float64Array(l),n=new PointSet(m),o=new PointVisibilityMap(h,k);e.forEach(function(a){a.svgPath=computeConnectorSvg(a,n,o);}), b.overallTime=new Date-c, b.numRoutingPoints=h.count(), b.numBoundaryLines=k.count(), postMessage({connectorDescriptors:e,metrics:b});};
